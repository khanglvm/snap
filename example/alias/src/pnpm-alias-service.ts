...
import { ensureFileExists, normalizeTrailingNewlines } from './shared/file-utils.js';

const PNPM_ALIAS_BLOCK_START = '# >>> pnpm-dev-aliases >>>';
const PNPM_ALIAS_BLOCK_END = '# <<< pnpm-dev-aliases <<<';
...
dev-add() {
    ...
    echo "" && dev-install
    echo "\${G}Done: \${PM} lockfile updated, installed via pnpm\${N}"
}
...
const hasPnpmAliasBlock = (content: string): boolean => content.includes(PNPM_ALIAS_BLOCK_START);

const removePnpmAliasBlock = (content: string): string => {
  const pattern = /# >>> pnpm-dev-aliases >>>[\s\S]*?# <<< pnpm-dev-aliases <<<\n?/g;
  return content.replace(pattern, '');
};

export interface InstallPnpmAliasesInput {
  overwrite?: boolean;
}

export interface InstallPnpmAliasesResult {
  ok: boolean;
  profilePath?: string;
  summary?: string;
  details?: string[];
...
export const installPnpmAliases = async (
  input: InstallPnpmAliasesInput = {}
): Promise<InstallPnpmAliasesResult> => {
  ...
  const exists = hasPnpmAliasBlock(current);
  ...
  if (exists && !overwrite) {
    return {
      ...
      conflict: true,
      profilePath,
      errorMessage: 'pnpm aliases already exist.'
    };
  }

  let next = current;
  if (exists && overwrite) {
    next = removePnpmAliasBlock(next);
  }

  next = normalizeTrailingNewlines(`${next}\n${PNPM_ALIASES_TEMPLATE}`);
  await fs.writeFile(profilePath, next, 'utf8');

  return {
    ok: true,
    profilePath,
    summary: 'Installed stealth pnpm aliases.',
    details: ['aliases=di,da,dr', `shell_profile=${profilePath}`]
  };
};
